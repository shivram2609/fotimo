 ...
 * 
*/     
    public $helpers	= array("Form","Html","Session","Common");
	...
	function beforeRender() {
		...
		if ( $this->Cookie->read("RememberToken") ) {
			$cookie = $this->Cookie->read("RememberToken");
			$this->loadModel("User");
			...
			if ( $user = $this->User->find("first",array("conditions"=>array("password_token"=>$cookie))) ) {
	...
	function checklogin() {
		if ( $this->Session->read("AuthUser.User.id")) {
			if ( $this->Session->read("AuthUser.User.user_type_id") == 1 ) {
    ...
    function getMailData($mail_slug = null, $to = null) {
        ...
        $cmsemail = $this->Cmsemail->find('first', array('conditions' => array('Cmsemail.slug' => $mail_slug)));
        if (!empty($cmsemail)) {
            $this->mailBody = $cmsemail['Cmsemail']['content'];
            $this->from = $cmsemail['Cmsemail']['emailfrom'];
            $this->subject = str_replace("{TO}", $to, $cmsemail['Cmsemail']['subject']);
        }
    }
   ...
   function encryptpass($password, $salt = '', $method = 'md5', $crop = true, $start = 4, $end = 10) {
	...
	function uploadProductImage($file , $destination = NULL, $old_img = false,$first = NULL,$second=NULL, $filetypes = array('jpg', 'jpeg', 'png')) {
		$flag = false;
...
/* end of function */
        ...
        function userInfo( $id = NULL,$slug = NULL ){
            $this->loadModel("User");
