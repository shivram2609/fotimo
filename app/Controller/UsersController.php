 ...
 */
	var $conditions = array();
		...
		$this->allowed = array('login','profile','signup','logout','loginwith','logoutwith','forgotpassword',"confirmation","admin_logout");
		parent::beforefilter();
		$this->Cookie->name = 'baker_id';
		$this->Cookie->time = 360000000;  // or '1 hour'
    ...
    function __login($identifier = NULL, $loginToken = NULL) {
                 $this->userInfo();
			...
			if ( $user = $this->User->find("first",array("conditions"=>array("email"=>$userName,"password"=>$password))) ) {
				if ( $user['User']['status'] == 1 ) {
					...
					if (isset($this->data['User']['remember_me']) && !empty($this->data['User']['remember_me']) ) {
						...
						$passwordToken = $this->encryptpass($user['User']['email'],$salt);
						$user1['User']['password_token'] = $passwordToken;
						$this->User->create();
						$this->User->id = $user['User']['id'];
						$this->User->save($user1);
						$this->Cookie->write("RememberToken",$passwordToken,false);
					}
    	...
    	function forgotpassword( ) {
    		if ($this->request->is("post")) {
				if ( isset($this->request->data['User']['email']) ) {
					if ( $user = $this->User->find("first",array("conditions"=>array("email"=>$this->request->data['User']['email'],"identifier"=>'',"status"=>1),"recursive"=>-1)) ) {
						echo $user1['User']['password_token'] = $otp = $this->getotp();
						$this->User->create();
						$this->User->id = $user['User']['id'];
						if ( $this->User->save($user1) ) {
							$this->getMailData("FORGOT_PASSWORD");
							$this->mailBody = str_replace("{OTP}",$otp,$this->mailBody);
							$this->sendMail($user['User']['email']);
							$this->Session->setFlash("Thanks for your request, Please check your email for otp to proceed.", 'default', array("class"=>"success_message"));
							$this->set(compact("otp"));
						} else {
							$this->Session->setFlash("Some error occured, Please try again.");
						}
					} else {
						$this->Session->setFlash("Invalid user.");
					}
					
				} elseif ( isset($this->request->data['User']['otp']) ) {
					$this->set("otp","1");
					if ( $user =  $this->User->findByPasswordToken($this->request->data['User']['otp']) ) {
						$this->User->set($this->data);
						if ( $this->User->validates() ) {
							$user1['User']['password'] = $this->encryptpass($this->data['User']['password']);
							$this->User->create();
							$this->User->id = $user['User']['id'];
							if ( $this->User->save($user1,array("validate"=>false)) ) {
								$this->Session->setFlash("Your password has been updated successfully.",'default',array("class"=>"success_message"));
								$this->redirect(SITE_LINK."login");
							} else {
								$this->Session->setFlash("An error ocuurred, Please try again.");
							}
						}
					} else {
						$this->Session->setFlash("Invalid user");
		...
		function getotp() {
			...
			if ( $this->User->findByPasswordToken($otp) ) {
				$this->getotp();
			} else {
				return $otp;
			}
	...
	function logout() {
		$loginReferrer = $this->referer();
		if ( $this->Session->read("Auth.User.identifier") != '' ) {
			$this->logoutwith($this->Session->read("AuthUser.User.loginfrom"));
		}
		$this->Session->write('login_referrer',$loginReferrer); 
                $this->Cookie->delete('AuthUser');
                $this->Cookie->destroy();
	...
	function dashboard() {
        ...
        function follow(){
            ...
            if ( $this->request->is('ajax') ) {
                ...
                if ( $follower = $this->Follower->find("first",array("conditions"=>array("user_id"=>$data['uid'],"follower_id"=>$data['follower_id']),"recursive"=>-1)) ) 
                 {
                    ...
                    $follower['Follower']['user_id'] = $data['uid'];
                    $follower['Follower']['follower_id'] = $data['follower_id'];
